<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      if ($registrationState === "registered") {
        console.log(
          "DEBUG:[LOGIN/NAVIGATION] Already registered, redirecting to dialer"
        );
        goto("/");
        return;
      }
      ...
      console.log(
        "DEBUG:[LOGIN/AUTO_LOGIN] Attempting auto-login with saved credentials"
      );
      const autoLoginSuccess = await authStore.autoLogin();

      if (autoLoginSuccess) {
        console.log(
          "DEBUG:[LOGIN/AUTO_LOGIN] Auto-login initiated, waiting for registration"
        );
        // Registration state polling will handle the redirect when registration completes
      } else {
        console.log(
          "DEBUG:[LOGIN/AUTO_LOGIN] No saved credentials or auto-login failed, showing login form"
        );
      }
    } catch (error) {
      console.error(
        "DEBUG:[LOGIN/NAVIGATION] Error during auto-login check",
        error
      );
  ...
  $effect(() => {
    if ($registrationState === "registered") {
      console.log(
        "DEBUG:[LOGIN/NAVIGATION] Registration successful, redirecting to dialer"
      );
      goto("/");
    }
  });
